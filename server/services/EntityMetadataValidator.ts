...
export module Services {
    export class EntityMetadataValidator<T> implements Contract.Services.IValidator<T> {
        ...
        public Validate(object:any, callback:(validationErrors:ClientErrorsModel.Model.ClientErrorsModel)=>void):void {
            ...
            if (!object) {
                var emptyMetadataErrorsModel:ClientErrorsModel.Model.ClientErrorsModel = ClientErrorsModel.Model.ClientErrorsModel.CreateWithError('EmptyMetadataValidation', null);
            ...
            if (!object.IconClassName) {
                var noIconErrorsModel:ClientErrorsModel.Model.ClientErrorsModel = ClientErrorsModel.Model.ClientErrorsModel.CreateWithError('NoIconMetadataValidation', null);
            ...
            if (!object.EntityName || !object.EntitySystemName || !entitySystemNameRegExp.test(object.EntitySystemName)) {
                var invalidSystemNameErrorsModel:ClientErrorsModel.Model.ClientErrorsModel = ClientErrorsModel.Model.ClientErrorsModel.CreateWithError('InvalidSystemNameMetadataValidation', null);
                ...
                metadataRepository.FindByCondition({EntitySystemName: object.EntitySystemName}, requestContext, function (data:any[], errors:ClientErrorsModel.Model.ClientErrorsModel) {
                    if (errors) {
                        ...
                        var invalidSystemNameErrorsModel:ClientErrorsModel.Model.ClientErrorsModel = ClientErrorsModel.Model.ClientErrorsModel.CreateWithError('ExistingSystemNameMetadataValidation', [object.EntitySystemName]);
                        callback(invalidSystemNameErrorsModel);
                    }
                    else {
                        //Everything is valid
